...
const chkFisica = document.getElementById('chkFisica');
const chkEmocional = document.getElementById('chkEmocional');

btn.addEventListener('click', () => {
    let opciones = [];
    if (chkFisica.checked) {
        opciones = opciones.concat(escenariosFisicos);
    }
    if (chkEmocional.checked) {
        opciones = opciones.concat(escenariosEmocionales);
    }
    if (opciones.length === 0) {
        output.textContent = 'Por favor, seleccione al menos un tipo de escenario.';
        return;
    }
    const index = Math.floor(Math.random() * opciones.length);
    output.textContent = opciones[index];
});
